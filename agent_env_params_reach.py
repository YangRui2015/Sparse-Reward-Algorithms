...
def design_agent_and_env(FLAGS):
    ...
    FLAGS.total_steps = 20                                                     # 每个episode最大步数
    ...
    env_params["action_bounds"] = np.array([1, 1, 1, 1])  # low-level action bounds
    env_params["action_offset"] = np.array([0, 0, 0, 0])  # Assumes symmetric low-level action ranges

    env_params["initial_qpos"] = {               # 初始状态
        'robot0:slide0': 0.4049,
        'robot0:slide1': 0.48,
        'robot0:slide2': 0.0,
    }
    env_params["block_gripper"] = True
    env_params["gripper_extra_height"] = 0.2
    env_params["project_state_to_end_goal"] = lambda state: state[0:3]    # 状态到目标goal的映射，只取位置信息
    ...
    env_params["project_state_to_subgoal"] = lambda state: state[0:3]      # 状态到子目标subgoal的映射，只取位置信息
    ...
    env_params["target_in_the_air"] = True
